...
def send_get_nextRequest(agent_ip, request_id: int) -> None:
    sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM, socket.IPPROTO_UDP)

    packet_builder = SNMPPacketBuilder()
    packet = packet_builder.build_get_next_request(request_id)
    print(f"{packet}")

    sock.sendto(packet, (agent_ip, PORT))
    sock.close()

def send_setRequest(agent_ip, oid: List[int], request_id: int , value) -> None:
    sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM, socket.IPPROTO_UDP)

    packet_builder = SNMPPacketBuilder()
    packet = packet_builder.build_set_request(request_id, oid, value)
...
def start_get_next_thread(value_var, manager_ip, agent_ip, request_id):
    send_get_nextRequest(agent_ip, request_id)
    response_msg = receive_getResponse(manager_ip)
    value_var.set(response_msg)

def start_set_thread(manager_ip, agent_ip, oid, request_id, value):
    send_setRequest(agent_ip, oid, request_id, value)
    response_msg = receive_getResponse(manager_ip)
